...
class ReportGenerator(Node):
    ...
    def __init__(self):
        ...
        self.report_generator_callback(None)
    ...
    def generate_pdf_report(self):
        ...
        for idx, point in enumerate(self.filtered_data):
            odom_coords = f"Odom Coordinates: ({round(point['odom_coords'].x*100,3)}, {round(point['odom_coords'].y*100,3)})"
            map_coords = f"Map Coordinates: ({round(point['x_map']*100,3)}, {round(point['y_map']*100,3)})"
    ...
    def report_generator_callback(self, msg):
        ...
        self.draw_marker(0.0, 0.0, 2)
        # # 13
        # self.draw_marker(-0.03648001646144066, -1.0705444281502416, 1)
        # # 8
        # self.draw_marker(0.9135459895816211, -0.06322340835189566, 1)
        for point in self.filtered_data:
